...
def _build_callbacks(cfg: dict, checkpoint_path: Path, logger_enabled: bool) -> list:
    """Create the Lightning callbacks compatible with the trainer logger setting."""
    callbacks = [
        EarlyStopping(
            monitor="val_loss",
...
def train_tft(train_ds, val_ds, cfg: dict):
    ...
    logger_enabled = bool(cfg["tft"].get("enable_logger", False))
    callbacks = _build_callbacks(cfg, checkpoint_path, logger_enabled)
    trainer = pl.Trainer(
